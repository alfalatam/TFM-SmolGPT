...
from smolgpt.config import GPTConfig
...
def setup_device(args):
    ...
    ctx = torch.autocast(device, dtype=dtype) if device == "cuda" else torch.no_grad()
...
def main():
    args = parse_args()
    # añadimos un aviso de si se esta usando cpu o cuda
    device =  "cuda" if torch.cuda.is_available() else "cpu"
    
    if device == "cpu":
        print("=======[WARNING] CUDA not avalaible, using CPU instead=======")
    ...
    x = torch.tensor(
        ...
        encode(args.prompt), dtype=torch.long, device=device
    ).unsqueeze(0)
    ...
    with torch.no_grad():
        with ctx:
            for k in range(args.num_samples):
                ...
                print(decode(y[0].tolist()).replace("\\n", "\n"))
                print("------------------")
