...
def train_vocab(vocab_size: int) -> None:
    ...
    # Aqui creamos un archio temporal a partir del dataset en formato txt para entrenar el vocabulario.
    tiny_file = DATA_CACHE_DIR / "Jan_Erik_Solem.txt"
...
# ======================= Definimos aqui una funcion para pteparar nuestro propio vocabulario a partir del corpus==================
# es una adaptacion de la funcion anterior (train vocab)

def train_vocab_txt(vocab_size: int, txt_dir: str):

    # buscamos los txt y creamos el archivo temporal
    txt_dir = Path(__file__).resolve().parents[2] / txt_dir
    txt_files = sorted(txt_dir.glob("*.txt"))

    temp_file = DATA_CACHE_DIR / "corpus.txt"
    
    if not txt_files:
        raise RuntimeError(f"No se han encontrado archivos de texto plano (txt) en la ruta {txt_dir}")
    
    # unimos los txt si los hubiese
    ...
    # este bloque lo he reusado del train vocab de tinystories
    spm.SentencePieceTrainer.train(
...
if __name__ == "__main__":
    ...
    # === añadimos para el txt  ======================
    
    txt_parser = subparsers.add_parser(
        "train-vocab-txt", help = "Entrenar vocabulario a partir de los txt" 
        )
    ...
    # añadimos el del txt
    elif args.command == "train-vocab-txt":
